...
static const char* mylogger_spec[] = {
    "implementation_id", "LArTpcLogger",
    "type_name",         "LArTpcLogger",
...
LArTpcLogger::LArTpcLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("lartpclogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("lartpclogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("LArTpcLogger");
}

LArTpcLogger::~LArTpcLogger()
{
}

RTC::ReturnCode_t LArTpcLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "LArTpcLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t LArTpcLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int LArTpcLogger::daq_dummy()
{
    return 0;
}

int LArTpcLogger::daq_configure()
{
...
int LArTpcLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "LArTpcLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "LArTpcLogger: Save to file: false\n";
            }
        }
...
int LArTpcLogger::daq_unconfigure()
{
    std::cerr << "*** LArTpcLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int LArTpcLogger::daq_start()
{
    std::cerr << "*** LArTpcLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: LArTpcLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** LArTpcLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int LArTpcLogger::daq_stop()
{
    std::cerr << "*** LArTpcLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "LArTpcLogger::stop: close files \n";
        }
...
int LArTpcLogger::daq_pause()
{
    std::cerr << "*** LArTpcLogger::pause" << std::endl;
    return 0;
}

int LArTpcLogger::daq_resume()
{
    std::cerr << "*** LArTpcLogger::resume" << std::endl;
    return 0;
}

int LArTpcLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** LArTpcLogger::InPort flushed\n";
    }
    return 0;
}

void LArTpcLogger::toLower(std::basic_string<char>& s)
{
...
int LArTpcLogger::daq_run()
{
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "LArTpcLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void LArTpcLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<LArTpcLogger>,
                                 RTC::Delete<LArTpcLogger>);
    }
};
