...
static const char* lartpcmonitor_spec[] =
{
    "implementation_id", "LArTpcMonitor",
    "type_name",         "LArTpcMonitor",
    "description",       "LArTpcMonitor component",
    "version",           "1.0",
...
LArTpcMonitor::LArTpcMonitor(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("lartpcmonitor_in",   m_in_data),
      ...
      m_debug(false)
{
    ...
    // Set InPort buffers
    registerInPort ("lartpcmonitor_in",  m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("LARTPCMONITOR");
}

LArTpcMonitor::~LArTpcMonitor()
{
}

RTC::ReturnCode_t LArTpcMonitor::onInitialize()
{
    if (m_debug) {
        std::cerr << "LArTpcMonitor::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t LArTpcMonitor::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int LArTpcMonitor::daq_configure()
{
    std::cerr << "*** LArTpcMonitor::configure" << std::endl;
...
int LArTpcMonitor::parse_params(::NVList* list)
{
...
int LArTpcMonitor::daq_unconfigure()
{
    std::cerr << "*** LArTpcMonitor::unconfigure" << std::endl;
    if (m_canvas) {
        delete m_canvas;
        m_canvas = 0;
    }
...
int LArTpcMonitor::daq_start()
{
    std::cerr << "*** LArTpcMonitor::start" << std::endl;
...
int LArTpcMonitor::daq_stop()
{
    std::cerr << "*** LArTpcMonitor::stop" << std::endl;
...
int LArTpcMonitor::daq_pause()
{
    std::cerr << "*** LArTpcMonitor::pause" << std::endl;

    return 0;
}

int LArTpcMonitor::daq_resume()
{
    std::cerr << "*** LArTpcMonitor::resume" << std::endl;

    return 0;
}

int LArTpcMonitor::reset_InPort()
{
    int ret = true;
    while(ret == true) {
        ret = m_InPort.read();
...
int LArTpcMonitor::decode_data(const unsigned char* mydata)
{
...
int LArTpcMonitor::fill_data(const unsigned char* mydata, const int size)
{
...
unsigned int LArTpcMonitor::read_InPort()
{
...
int LArTpcMonitor::daq_run()
{
    if (m_debug) {
        std::cerr << "*** LArTpcMonitor::run" << std::endl;
    }
...
extern "C"
{
    void LArTpcMonitorInit(RTC::Manager* manager)
    {
        RTC::Properties profile(lartpcmonitor_spec);
        manager->registerFactory(profile,
                    RTC::Create<LArTpcMonitor>,
                    RTC::Delete<LArTpcMonitor>);
    }
};
